...
function main()
    ...
    println("[progress] case-ready: token spec built (tokens=", T, ", chain_type=", chain_type, ")")
    ...
    println(
        "[progress] case-ready: optional inputs resolved (msa_file=",
        (isempty(msa_file) ? "none" : msa_file),
        ", templates=",
        length(template_paths),
        ")",
    )
